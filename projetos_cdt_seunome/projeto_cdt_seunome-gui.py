...
class AplicaçãoHamburgueria:
    ...
    def mostrar_tela_login(self):
        ...
        # Título
        lbl_titulo = tk.Label(self.root, text="🍔 Hamburgueria CDT", font=("Helvetica", 20, "bold"), fg="#f39c12", bg="#2c3e50")
        ...
        # Campos de texto
        tk.Label(frame, text="E-mail:", font=("Arial", 11), fg="white", bg="#34495e").grid(row=0, column=0, sticky="w", pady=5)
        self.ent_email = tk.Entry(frame, width=30, font=("Arial", 11))
        self.ent_email.grid(row=0, column=1, pady=5)

        tk.Label(frame, text="Senha:", font=("Arial", 11), fg="white", bg="#34495e").grid(row=1, column=0, sticky="w", pady=5)
        self.ent_senha = tk.Entry(frame, width=30, font=("Arial", 11), show="*")
        self.ent_senha.grid(row=1, column=1, pady=5)

        # Botões
        btn_login = tk.Button(frame, text="Entrar", font=("Arial", 10, "bold"), bg="#27ae60", fg="white", width=12, command=self.executar_login)
        btn_login.grid(row=2, column=1, sticky="e", pady=15)

        btn_cadastrar = tk.Button(self.root, text="Criar Nova Conta", font=("Arial", 10), bg="#e67e22", fg="white", command=self.mostrar_tela_cadastro)
        btn_cadastrar.pack(pady=10)

    def mostrar_tela_cadastro(self):
        self.limpar_tela()

        lbl_titulo = tk.Label(self.root, text="📝 Cadastro de Usuário", font=("Helvetica", 18, "bold"), fg="#f39c12", bg="#2c3e50")
        lbl_titulo.pack(pady=20)

        frame = tk.Frame(self.root, bg="#34495e", padx=20, pady=20)
        frame.pack(pady=10)

        tk.Label(frame, text="Nome Completo:", font=("Arial", 11), fg="white", bg="#34495e").grid(row=0, column=0, sticky="w", pady=5)
        self.ent_nome_cad = tk.Entry(frame, width=30, font=("Arial", 11))
        self.ent_nome_cad.grid(row=0, column=1, pady=5)

        tk.Label(frame, text="E-mail:", font=("Arial", 11), fg="white", bg="#34495e").grid(row=1, column=0, sticky="w", pady=5)
        self.ent_email_cad = tk.Entry(frame, width=30, font=("Arial", 11))
        self.ent_email_cad.grid(row=1, column=1, pady=5)

        tk.Label(frame, text="Senha:", font=("Arial", 11), fg="white", bg="#34495e").grid(row=2, column=0, sticky="w", pady=5)
        self.ent_senha_cad = tk.Entry(frame, width=30, font=("Arial", 11), show="*")
        self.ent_senha_cad.grid(row=2, column=1, pady=5)

        btn_salvar = tk.Button(frame, text="Salvar Cadastro", font=("Arial", 10, "bold"), bg="#27ae60", fg="white", command=self.executar_cadastro)
        btn_salvar.grid(row=3, column=1, sticky="e", pady=15)

        btn_voltar = tk.Button(self.root, text="Voltar ao Login", font=("Arial", 10), bg="#95a5a6", fg="white", command=self.mostrar_tela_login)
    ...
    def mostrar_tela_pedidos(self):
        ...
        # Cabeçalho
        frame_topo = tk.Frame(self.root, bg="#2c3e50")
        frame_topo.pack(fill="x", px=10, pady=10)
        
        lbl_welcome = tk.Label(frame_topo, text=f"Cliente: {self.usuario_logado['nome']}", font=("Arial", 12, "bold"), fg="white", bg="#2c3e50")
        lbl_welcome.pack(side="left", padx=10)

        btn_logout = tk.Button(frame_topo, text="Sair", font=("Arial", 9), bg="#e74c3c", fg="white", command=self.mostrar_tela_login)
        btn_logout.pack(side="right", padx=10)

        # Conteúdo em duas colunas (Cardápio na esquerda, Carrinho na direita)
        frame_conteudo = tk.Frame(self.root, bg="#2c3e50")
        frame_conteudo.pack(fill="both", expand=True, padx=10, pady=5)

        # Coluna Esquerda: Cardápio
        frame_cardapio = tk.LabelFrame(frame_conteudo, text="🍔 Cardápio", font=("Arial", 11, "bold"), fg="#f39c12", bg="#34495e", padx=10, pady=10)
        ...
        frame_carrinho = tk.LabelFrame(frame_conteudo, text="🛒 Carrinho", font=("Arial", 11, "bold"), fg="#f39c12", bg="#34495e", padx=10, pady=10)
        frame_carrinho.pack(side="right", fill="both", expand=True, padx=5)

        self.lst_carrinho = tk.Listbox(frame_carrinho, font=("Arial", 10), height=10)
        self.lst_carrinho.pack(fill="both", expand=True, pady=5)

        self.lbl_total = tk.Label(frame_carrinho, text="Total: R$ 0.00", font=("Arial", 12, "bold"), fg="white", bg="#34495e")
        self.lbl_total.pack(pady=5)

        btn_checkout = tk.Button(frame_carrinho, text="Finalizar Pedido", font=("Arial", 11, "bold"), bg="#27ae60", fg="white", command=self.finalizar_pedido)
